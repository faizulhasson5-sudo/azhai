...
Comp.relatedPosts = function(currentSlug){
    ...
    <div class="blog-grid">${posts.map(p=>`
        <a href="/blog/${p.slug}.html" class="blog-card">
            <div class="blog-card-img" style="background:var(--primary-light);height:160px;display:flex;align-items:center;justify-content:center;font-size:2rem">${p.category==='SEO Tools'?'🔍':p.category==='SEO Tips'?'📈':p.category==='Development'?'💻':p.category==='Blogging'?'✍':'📱'}</div>
